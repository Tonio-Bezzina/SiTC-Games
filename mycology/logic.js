(function (root, factory) {
  ...
  function sanitize(raw) {
    if (!raw || typeof raw!=="object" || ![1,2,3,4,5,6,7,8,9,VERSION].includes(raw.version) || !DIFFICULTIES.includes(raw.difficulty) || !/^MYC-\d{4}$/.test(raw.caseId||"")) return null;
  ...
  function orderCorrect(order){return Array.isArray(order)&&order.join("|")===JOURNEY.join("|");}
  function finishMission8(state){if(!orderCorrect(state?.missionState?.[8]?.order))return false;state.missionFlags[8]=true;state.currentMission=8;return true;}
  function canAwardHub(state){return Boolean(state&&state.version===VERSION&&/^MYC-\d{4}$/.test(state.caseId)&&DIFFICULTIES.includes(state.difficulty)&&JOURNEY.every((_,i)=>state.missionFlags?.[i+1]===true)&&state.clues?.safety==="complete"&&state.clues?.patient==="Jamie Borg"&&state.clues?.specimen==="skin scrapings"&&state.clues?.reception==="accepted MYC-2048 skin scraping"&&state.clues?.microscopy==="fungal elements seen"&&state.clues?.culture==="growth-positive fluffy powdery colony"&&state.clues?.identification==="Aspergillus fumigatus"&&state.clues?.mic==="well 5, first no-visible-growth endpoint");}
  ...
  return {VERSION,SAMPLE_TARGETS,DIFFICULTIES,QUESTIONS,MATCHES,RECEPTION_CASES,SPECIES,JOURNEY,caseId,freshState,sanitize,answerQuestion,shouldAutoHint,canCompleteMission1,finishMission1,matchSample,finishMission2,receptionDecision,finishMission3,allocationCounts,allocationReady,samplePlacement,microscopyTarget,finishMission4,plateOrder,finishMission5,keyChoice,finishMission6,isMicWell,finishMission7,orderCorrect,finishMission8,canAwardHub,awardProgress,earliestIncomplete};
});
