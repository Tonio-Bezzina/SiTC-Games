(function(){
  ...
  function renderMission4(){
    const m=state.missionState[4];if(state.missionFlags[4]){renderMissionComplete(4,"Sample processed","Microscopy clue saved. The culture plate is now in the simulated incubator.","Continue to Mission 5",()=>announce("Mission 5 is the next stage of the journey."));return;}
  ...
  function route(){if(!state)return renderLevel();if(state.currentMission===1)return renderMission1();if(state.currentMission===2)return renderMission2();if(state.currentMission===3)return renderMission3();if(state.currentMission===4)return renderMission4();renderMission4();}
