(function(){
  ...
  function renderMission4(){
    ...
    if(m.stage==="allocate"){const counts=L.allocationCounts(m.destinations);chrome(`<main class="mission-scene" id="main"><header class="mission-title"><p class="eyebrow">MISSION 4 OF 8 · PROCESS THE DERMATOLOGY SAMPLE</p><h1>Split the sample between two methods</h1><p>Let’s use the skin scraping sample for fluorescent microscopy and culture. Place three pieces on the microscope slide and three pieces on the SDCC fungal agar plate.</p></header><section class="activity-card"><div class="method-art"><img src="assets/mission-4/microscope-slide.svg" alt="Prepared microscope slide"><img src="assets/mission-4/sdcc-plate.svg" alt="Simulated SDCC fungal culture plate"></div><p class="count-status" aria-live="polite">Slide: ${counts.slide} of 3 · Culture: ${counts.culture} of 3</p><div class="flake-grid">${Array.from({length:6},(_,i)=>{const n=i+1,d=m.destinations[n];return `<fieldset><legend>Skin scraping piece ${n}${d?` — on ${d}`:""}</legend><img src="assets/mission-4/skin-scraping-piece.svg" alt=""><button data-piece="${n}" data-place="slide" ${counts.slide>=3&&d!=="slide"?"disabled":""}>Place on slide</button><button data-piece="${n}" data-place="culture" ${counts.culture>=3&&d!=="culture"?"disabled":""}>Place on culture</button></fieldset>`;}).join("")}</div><p class="feedback ${feedbackType}">${feedback}</p>${L.allocationReady(m.destinations)?`<div class="success-panel"><p><strong>Good work.</strong> Three pieces are ready for microscopy and three are ready for culture.</p><button class="primary-button" data-microscope>Use the microscope</button></div>`:""}</section></main>`,4);app.querySelectorAll("[data-place]").forEach(b=>b.addEventListener("click",()=>{m.destinations[b.dataset.piece]=b.dataset.place;save();renderMission4();announce(`Piece ${b.dataset.piece} placed on ${b.dataset.place}.`);}));app.querySelector("[data-microscope]")?.addEventListener("click",()=>{m.stage="microscopy";save();renderMission4();});}
    else{chrome(`<main class="mission-scene" id="main"><header class="mission-title"><p class="eyebrow">MISSION 4 OF 8 · MICROSCOPY</p><h1>Find the bright fungal structures</h1><p>Place the prepared slide on the microscope, adjust the focus, then explore the field to find bright fungal structures.</p></header><section class="activity-card microscope-card"><img class="microscope-field" src="assets/mission-4/fluorescence-field.svg" alt="Stylised fluorescent microscope field with several regions"><label for="focus">Focus <output>${m.focus}%</output></label><input id="focus" type="range" min="0" max="100" value="${m.focus}"><div class="region-grid"><button data-region="debris">Explore upper field</button><button data-region="branching">Explore bright branching field</button><button data-region="empty">Explore lower field</button></div><p class="feedback ${feedbackType}">${feedback}</p><p class="hint-text" ${m.hintShown?"":"hidden"}>Look for bright branching, tube-like hyphae with cross-walls and small spore shapes.</p>${m.found?`<div class="success-panel"><p>You found them! The bright structures are fungal elements. Fluorescence makes it easier for us to find them.</p><button class="primary-button" data-finish4>Save microscopy clue</button></div>`:""}</section></main>`,4);app.querySelector("#focus").addEventListener("input",e=>{m.focus=Number(e.target.value);save();renderMission4();});app.querySelectorAll("[data-region]").forEach(b=>b.addEventListener("click",()=>{if(L.microscopyTarget(m.focus,b.dataset.region)){m.found=true;feedback="You found them! The bright structures are fungal elements. Fluorescence makes it easier for us to find them.";feedbackType="good";}else{m.attempts+=1;feedback="Look carefully through the microscope and try finding a different area.";feedbackType="try";}save();renderMission4();announce(feedback);}));app.querySelector("[data-hint]").addEventListener("click",()=>{m.hintShown=true;save();renderMission4();});app.querySelector("[data-finish4]")?.addEventListener("click",()=>{if(L.finishMission4(state)){save();renderMission4();}});}
  }

  function renderMission5(){
    const m=state.missionState[5];if(state.missionFlags[5]){renderMissionComplete(5,"Growth detected","Culture clue saved. Let’s compare the colony and microscopic appearance.","Continue to Mission 6",()=>announce("Mission 6 is the next stage of the journey."));return;}
  ...
  function route(){if(!state)return renderLevel();if(state.currentMission===1)return renderMission1();if(state.currentMission===2)return renderMission2();if(state.currentMission===3)return renderMission3();if(state.currentMission===4)return renderMission4();if(state.currentMission===5)return renderMission5();renderMission5();}
