...
const fluorescenceView = (blur) => `<rect width="1400" height="900" fill="#020A18"/><defs><filter id="soft" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="${blur}"/></filter><filter id="glow"><feGaussianBlur stdDeviation="7"/></filter></defs><g ${blur?`filter="url(#soft)"`:""} fill="none" stroke="#5BE9FF" stroke-linecap="round"><g opacity=".35" filter="url(#glow)" stroke-width="23"><path d="M160 720C330 660 390 430 560 410S790 230 1010 255M555 410l-75-145M760 320l110-150M350 580l-120-140"/></g><g stroke-width="10"><path d="M160 720C330 660 390 430 560 410S790 230 1010 255M555 410l-75-145M760 320l110-150M350 580l-120-140"/><path d="M263 674l-22-31m119-84-31-20m127-98-34-8m91-41-22-30m111-22-11-35m106 4 24-29" stroke="#B7FAFF" stroke-width="5"/></g><g fill="#9AF5FF" stroke="#E8FFFF" stroke-width="4">${[[1010,255],[1050,236],[1086,264],[870,170],[900,146],[930,180],[480,265],[448,240]].map(([x,y])=>`<circle cx="${x}" cy="${y}" r="15"/>`).join("")}</g></g>`;
[18,11,0,7,15].forEach((blur,index)=>writeSvg(`mission-4/microscopy-view-${index+1}.svg`, `${index===2?"Focused":"Out-of-focus"} simulated fluorescence microscope view ${index+1}`, 1400, 900, fluorescenceView(blur)));
...
const plateBase = appearance => `<circle cx="300" cy="300" r="245" fill="#EEF7FA" stroke="${C.navy}" stroke-width="12"/><circle cx="300" cy="300" r="212" fill="#E6D79A" stroke="#B79F55" stroke-width="8"/><path d="M135 220c80-68 265-82 355 0" fill="none" stroke="white" stroke-opacity=".55" stroke-width="16" stroke-linecap="round"/>${appearance}`;
writeSvg("mission-5/culture-plate-mould.svg", "Fluffy powdery spreading mould-like colony", 600, 600, plateBase(`<g fill="#5C786F" stroke="#E9F4EA" stroke-width="7">${[[300,300,125],[220,255,75],[390,350,82],[340,210,70],[235,385,62]].map(([x,y,r])=>`<circle cx="${x}" cy="${y}" r="${r}"/>`).join("")}</g>`));
writeSvg("mission-5/culture-plate-none.svg", "Culture plate with no visible growth", 600, 600, plateBase(`<circle cx="300" cy="300" r="22" fill="#D2C77C" opacity=".45"/>`));
writeSvg("mission-5/culture-plate-bacteria.svg", "Culture plate with smooth shiny spots", 600, 600, plateBase(`<g fill="#D8C6A0">${[[220,240],[360,225],[275,360],[400,350]].map(([x,y])=>`<circle cx="${x}" cy="${y}" r="25"/>`).join("")}</g>`));
writeSvg("mission-5/culture-plate-yeast.svg", "Culture plate with small creamy dots", 600, 600, plateBase(`<g fill="#C89471">${[[205,290],[275,215],[385,255],[380,390],[255,400]].map(([x,y])=>`<circle cx="${x}" cy="${y}" r="34"/>`).join("")}</g>`));
...
async function main() {
  ...
  await contactSheet(["shared/dr-mira-neutral.png","shared/dr-mira-pointing.png","shared/dr-mira-success.png","mission-4/microscopy-view-1.svg","mission-4/microscopy-view-2.svg","mission-4/microscopy-view-3.svg","mission-4/microscopy-view-4.svg","mission-4/microscopy-view-5.svg","mission-5/culture-plate-mould.svg","mission-5/culture-plate-none.svg","mission-5/culture-plate-bacteria.svg","mission-5/culture-plate-yeast.svg","mission-6/aspergillus-colony-set.svg","mission-6/aspergillus-microscopy-set.svg"],"state-families.png",3,440,360);
