...
assert(L.orderCorrect(L.JOURNEY));assert(!L.orderCorrect(L.JOURNEY.slice().reverse()));const s8=L.freshState("junior",2048);s8.missionFlags={1:true,2:true,3:true,4:true,5:true,6:true,7:true};s8.missionState[8].order=L.JOURNEY.slice();assert(L.finishMission8(s8));Object.assign(s8.clues,{safety:"complete",patient:"Jamie Borg",specimen:"skin scrapings",reception:"accepted MYC-2048 skin scraping",microscopy:"fungal elements seen",culture:"growth-positive fluffy powdery colony",identification:"Aspergillus fumigatus",mic:"well 5, first no-visible-growth endpoint"});assert(L.canAwardHub(s8));const progress=L.awardProgress({completedCases:{histology:["main"]}});assert.deepEqual(progress.completedCases.mycology,["main"]);assert.deepEqual(progress.completedCases.histology,["main"]);assert.deepEqual(L.awardProgress(progress).completedCases.mycology,["main"]);
console.log("PASS mycology logic missions 1-8 and hub award");
